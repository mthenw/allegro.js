...
/* Custom filters from categories
 http://allegro.pl/webapi/documentation.php/show/id,90#method-input
 */
Client.prototype.doGetItemsList = function() {
  var self = this;
  self._soapClient.doGetItemsList({
    'webapiKey': self._key,
    'countryId': self._countryId,
    'filterOptions': [
      [{'item':
        {'filterId': 'category', 'filterValueId': {'item': ['237548']}}
      }]
    ]
  }, function (err, result) {
    if (err) {
      ...
      var categoriesList = result.categoriesList[0].categoryTree;
      var filtersList = result.filtersList[0].item;
      console.log(result);
